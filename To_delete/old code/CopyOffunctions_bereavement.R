...
getDeathRates <- function(df, sim.id, country, scenario,
  ...
  if(get_death_rate){
    ...
    deaths_period <- sum(deaths_vector < month_range_high) #changed to remove =
...
getDeathRatesOnly <- function(sim.id, country, scenario, USER,
  ...
  #Save intermediate output
  save(data_death_rates, file = paste0("~/covid_simulation/Data/death_rates/", "data", sim.id, "_", country, "_", scenario, ".RData"))
