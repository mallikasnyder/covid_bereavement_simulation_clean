...
# Add unqieu id for each simulation

# nyumber of rows in each simulation 
age_sex_groups <- length(unique(adj.total$age))*2
...
for_assigning_id <-
  ...
  # filter(week == 5) %>% 
  mutate(
    rows = n * age_sex_groups
    , sim_num = n / age_sex_groups
    )
...
# Test that it matches numbe or rows per coutry
# for_assigning_id %>% 
#   group_by(country) %>% 
#   summarise(all = sum(rows))
  
# Assign unique id to each simulation
