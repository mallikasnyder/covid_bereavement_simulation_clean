...
getKin <- function(opop = opop, KidsOf = KidsOf, pid) {
  ...
  #Nuclear family
  res$nuclear <- mapply(c, res$parents, res$spouse, res$children, res$siblings, SIMPLIFY = F)
  #Extended family
  #res$extended <- mapply(c, res$nuclear, res$gparents, res$gunclesaunts, 
  #                       res$firstcousins, res$parentsofspouse, res$brothersisterinlaw, SIMPLIFY = F)
...
#-------Estimates function
getEstimates <- function(sim.id, country, scenario, 
                         feb2020 = feb2020, postcovid2020 = postcovid2020) {
  #First level needs
  folder <- paste0("/90days/mallikasnyder/", scenario, "RESULTS/", "SocCovid-mallikasnyder/", country, ":Medium.sup")
  ...
  save(data, file = paste0("~/covid_simulation/Data/sim_results_fc/data", sim.id, "_", country, "_", scenario, ".RData"))
