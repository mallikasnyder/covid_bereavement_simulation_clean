...
#Load data
data <- get(load(file = "~/covid_bereavement_simulation_clean/Data/finaldata_monthly_v2.RData"))
```
```{r}
#List all files in the directory
allfiles <- list.files("~/covid_bereavement_simulation_clean/Data/death_rates/",  full.names = T)


#Load all files and put them into a single object
death_data <- lapply(1:length(allfiles), function(x) {
  try(load(file = allfiles[x]))
  output <- data_death_rates
  return(output)
})

flattened <- do.call(c, death_data)

death_data <- bind_rows(flattened)
```
...
stmf <- fread("~/covid_bereavement_simulation_clean/Rcode/stmf_sep28.csv", stringsAsFactors = F)
